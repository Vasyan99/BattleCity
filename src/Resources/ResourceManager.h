...
public:
	...
	static void unloadAllResources(); //вместо деструктора. Удаляет все объекты, которыми владеет ресурс менеджер
	...
	static std::shared_ptr<Renderer::Texture2D> loadTexture(const std::string& textureName, const std::string& texturePath);//для начала получим голые пиксели. Передадим название текстуры, а также где она будет лежать.
...
private:
	...
	//точно также будем подгружать спрайты.
	typedef std::map<const std::string, std::shared_ptr<Renderer::Sprite>> SpritesMap;
	static SpritesMap m_sprites;

	//отдельный мэп для AnimatedSprite
